<script lang="ts">
  ...
  function dragMove(e: PointerEvent) {
    ...
    app.win.y = Math.max(0, e.clientY - dy)
  }
  ...
  const style = $derived(
    app.win.maximized
      ? 'inset: 8px 8px 38px 8px; width: auto; height: auto;'
      : `left:${app.win.x}px;top:${app.win.y}px;width:${app.win.w}px;height:${app.win.h}px;`,
  )
</script>
...
<section
  ...
  style={style}
>
  <div class="caption" class:inactive={!app.connected} onpointerdown={dragStart}>
    <CatLogo compact />
    ...
    <div class="caption-btns">
      <button
        ...
        onclick={() => (app.win.minimized = true)}
      >
        ▬
      </button>
      <button
        ...
        title="Maximize"
        onpointerdown={(e) => e.stopPropagation()}
        onclick={() => (app.win.maximized = !app.win.maximized)}
      >
        {app.win.maximized ? '❐' : '□'}
      </button>
      <button
        ...
        title="Close"
        onpointerdown={(e) => e.stopPropagation()}
        onclick={() => (app.win.open = false)}>×</button
      >
    </div>
  </div>
  ...
  <div class="toolbar groove">
    {#each tools as t (t.id)}
      <button
        ...
        disabled={app.phase === 'setup'}
      >
        {#if t.id === 'chat'}
          <svg viewBox="0 0 32 32">
            <rect x="2" y="4" width="18" height="12" fill="#fff" stroke="#000" />
            <rect x="10" y="14" width="18" height="12" fill="#c6e4ff" stroke="#000" />
          </svg>
        {:else if t.id === 'library'}
          <svg viewBox="0 0 32 32">
            <path d="M4 10h10l2 3h12v13H4z" fill="#f4d060" stroke="#000" />
            <path d="M12 18c6 0 8 4 8 4s-1-8-8-8-8 8-8 8 2-4 8-4z" fill="#1e4ea8" />
          </svg>
        {:else if t.id === 'search'}
          <svg viewBox="0 0 32 32">
            <circle cx="12" cy="14" r="6" fill="none" stroke="#000" stroke-width="2" />
            <circle cx="20" cy="14" r="6" fill="none" stroke="#000" stroke-width="2" />
            <path d="M8 20h16" stroke="#000" />
            <path d="M10 22h12" stroke="#000" />
          </svg>
        {:else if t.id === 'hotlist'}
          <svg viewBox="0 0 32 32">
            <path d="M16 4c6 8 10 12 10 18a10 10 0 1 1-20 0c0-6 4-10 10-18z" fill="#e67a00" stroke="#000" />
          </svg>
        {:else}
          <svg viewBox="0 0 32 32">
            <path d="M6 10h8V6l8 8-8 8v-4H6z" fill="#2f7d32" stroke="#000" />
            <path d="M26 22h-8v4l-8-8 8-8v4h8z" fill="#1565c0" stroke="#000" />
          </svg>
        {/if}
        {t.label}
      </button>
